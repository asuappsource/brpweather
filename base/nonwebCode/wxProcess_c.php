...
function updateStations() {
    ...
    if ($result = $dbh->query('SELECT * FROM station')) {
      while ($row = $result->fetch(PDO::FETCH_BOTH)) {
            $stations[$row[0]] = array('name' => $row[2], 'dir' => $row[1]);
        }
    }
...
function watchDirectories() {
    ...
    while (($signal == 1) && (($events = inotify_read($inotify)) !== false)) {
        ...
        if (time() >= 1800 + $stations['lastUpdate']) updateStations($dbh);
    }
}
...
function parseFile($filename) {
    ...
    $wxData['reported_time'] = date('c', strtotime($wxData['date'].' '.$wxData['time']));
    unset($wxData['date']);
    unset($wxData['time']);
